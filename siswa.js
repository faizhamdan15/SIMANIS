...
function getFiltered() {
  ...
  return allStudents.filter(s => {
    ...
    const okGender = !gender || (gender === "EMPTY" ? !s.gender : s.gender === gender);
...
function render() {
  ...
  if (!slice.length) {
    ...
    $("studentTableBody").innerHTML = slice.map((s,i) => `
      <tr>
        <td>${start + i + 1}</td>
        <td><div class="student-name">${esc(s.full_name)}</div><div class="muted">${esc(s.id.slice(0,8))}</div></td>
        <td><div>${esc(s.nis || "-")}</div><div class="muted">${esc(s.nisn || "-")}</div></td>
        <td>${esc(s.gender || "-")}</td>
        <td><span class="badge">${esc(s.class_name || "Belum ada kelas")}</span></td>
        <td>${esc(s.roll_number ?? "-")}</td>
        <td><div class="row-actions"><button class="mini-btn" data-edit="${esc(s.id)}">Edit</button><button class="mini-btn danger" data-disable="${esc(s.id)}">Nonaktifkan</button></div></td>
      </tr>`).join("");

    $("studentCards").innerHTML = slice.map(s => `
      <article class="student-card">
        <div class="top"><div><h4>${esc(s.full_name)}</h4><p>${esc(s.nis || "NIS belum diisi")} · ${esc(s.nisn || "NISN belum diisi")}</p></div><span class="badge">${esc(s.class_name || "Belum ada kelas")}</span></div>
        <div class="student-meta"><div><b>JK</b><br>${esc(s.gender || "-")}</div><div><b>No. Urut</b><br>${esc(s.roll_number ?? "-")}</div></div>
        <div class="row-actions"><button class="mini-btn" data-edit="${esc(s.id)}">Edit</button><button class="mini-btn danger" data-disable="${esc(s.id)}">Nonaktifkan</button></div>
      </article>`).join("");
  }
  ...
  document.querySelectorAll("[data-disable]").forEach(b => b.addEventListener("click", () => disableStudent(b.dataset.disable)));
}
...
function openAdd() { resetForm(); $("modalTitle").textContent = "Tambah Siswa"; openModal(); }
...
async function saveStudent(e) {
  ...
  const payload = {
    full_name: $("fullName").value.trim(), nis: $("nis").value.trim() || null, nisn: $("nisn").value.trim() || null,
    gender: $("gender").value || null, admission_year: $("admissionYear").value ? Number($("admissionYear").value) : null,
    birth_place: $("birthPlace").value.trim() || null, birth_date: $("birthDate").value || null,
    father_name: $("fatherName").value.trim() || null, mother_name: $("motherName").value.trim() || null,
    guardian_name: $("guardianName").value.trim() || null, guardian_phone: $("guardianPhone").value.trim() || null,
    address: $("address").value.trim() || null, status: "AKTIF"
  };
  ...
  try {
    ...
    if (activeSemesterId) {
      if (enrollmentId) {
        ...
        await restWrite("student_enrollments", "POST", {student_id: finalStudentId,class_id: classId,semester_id: activeSemesterId,roll_number: roll,status: "AKTIF"});
      }
    }
