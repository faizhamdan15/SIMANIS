...
module.exports=async function(req,res){
  try{
    ...
    if(!slug){res.status(404).setHeader("Content-Type","text/html; charset=utf-8");return res.end(notFound())}

    const r=await findArticle(slug);
    if(!r){res.status(404).setHeader("Content-Type","text/html; charset=utf-8");return res.end(notFound())}
...
<img class="article-image" src="${esc(image)}" alt="${esc(imageAlt)}">
