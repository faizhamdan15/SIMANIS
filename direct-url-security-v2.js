(function(){
  ...
  const MODULE_BY_FILE={
    ...
    "security-access-check.html":"PENGATURAN"
  };

  const AUTH_ONLY=new Set(["account-security.html"]);
  const isUnit=file==="unit-kerja.html";
  const known=MODULE_BY_FILE[file]||AUTH_ONLY.has(file)||isUnit;
    ...
    el.innerHTML=`
      <div class="sg-card">
        ...
        <div id="sgActions" class="sg-actions" style="display:none">
          ...
          <button id="sgLogout" type="button">Keluar</button>
        </div>
      </div>`;
    const st=document.createElement("style");
    st.textContent=`
      #simanisSecurityGate{position:fixed;inset:0;z-index:2147483647;background:#f5f8f6;display:grid;place-items:center;padding:20px;font-family:Inter,Arial,sans-serif}
      #simanisSecurityGate .sg-card{width:min(410px,100%);background:#fff;border:1px solid #dbe7e0;border-radius:20px;padding:24px;text-align:center;box-shadow:0 20px 60px rgba(6,61,44,.13)}
      ...
      #simanisSecurityGate strong{display:block;color:#123a2a;font-size:15px}
      #simanisSecurityGate span{display:block;margin-top:7px;color:#6d7d75;font-size:10px;line-height:1.6}
      #simanisSecurityGate .sg-actions{margin-top:16px;gap:8px;justify-content:center}
  ...
  function denied(title,msg,api){
    ...
    const b=document.getElementById("sgLogout");
    b.onclick=async()=>{
  ...
  function allow(){
    document.getElementById("simanisSecurityGate")?.remove();
  }

  async function waitReady(){
    for(let i=0;i<100;i++){
      if(window.simanisReady)return await window.simanisReady;
  ...
  async function verify(){
    ...
    try{
      ...
      if(!p?.is_active){
        denied("Akun tidak aktif","Akun ini tidak diizinkan menggunakan halaman SIMANIS.",api);
        return;
      }

      if(AUTH_ONLY.has(file)){
        allow();
        return;
      }
      ...
      if(file==="wali-kelas.html"){
        ...
        if(!h?.has_homeroom){
          denied("Akses Wali Kelas ditolak","Halaman ini hanya untuk guru yang menjadi wali kelas aktif.",api);
          return;
        }
        allow();
        return;
      ...
      if(isUnit){
        ...
        const allowedUnits=new Set([
          "PKM_KURIKULUM","PKM_KESISWAAN","PKM_BENDAHARA_SARPRAS",
          "PKM_HUMASY","KEPALA_TU","KALAB_IPA","KALAB_BISNIS"
        ]);
        if(!allowedUnits.has(moduleCode)){
          denied("Unit kerja tidak valid","Kode unit pada alamat halaman tidak dikenali.",api);
          return;
        }
      }
      ...
      if(!allowed){
        denied(
          ...
          `Akun ${p.full_name||""} tidak memiliki hak untuk membuka ${moduleCode}.`,
          api
        );
        return;
      }
      ...
      denied(
        "Verifikasi akses gagal",
        "Hak akses halaman tidak dapat diverifikasi. Muat ulang halaman. Jika tetap terjadi, hubungi administrator.",
        api
      );
    }
  }

  if(document.body)verify();
  else document.addEventListener("DOMContentLoaded",verify,{once:true});
})();